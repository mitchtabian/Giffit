...
//    }
    ...
    private fun Context.saveGif(
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.Q) {
            emitAll(saveGifToScopedStorage(bytes))
        } else {
            // Scoped storage doesn't exist before Android 29 so need to check permissions
            if (checkFilePermissions()) {
                emitAll(saveGifToStorage(bytes))
            } else {
                launchPermissionRequest() // TODO("Probably need to emit this too somehow then launch in VM")
            }
        }
    }
